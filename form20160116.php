...
//  $brnwhr = " where creditor_id=0";
  ...
  $brnwhr = " where c2.id=" . $_POST['crdd'];
	...
	$crdd = $_POST['crdd'];
//				if (empty($crdd)) {
//					$brnwhr = " where branch_id in (select branch_id from transdet where cred_id = 0) ";
//				} else {
//					$brnwhr = " where branch_id in (select branch_id from transdet where cred_id = " . $crdd . ") ";
//				}
/*
...
<?php
			...
			//	MODIFY
			//	MODIFY
	echo "<h1>transdet modify form</h1>";
...
//		echo "anything";
		echo "<tr><td>" . $row['tid'] . "</td><td>" . $row['tdt'] . "</td><td>" . $row['crd'] . "</td><td>" . $row['dsc'] . "</td><td>" . $row['tty'] . "</td><td>" . $row['acc'] . "</td><td>" . $row['amt'] .
  			...
  			<td><a href=\"form.php?tid=" . $row['tid'] . "\">mod</a></td><td><a href=\"post.php?tid=" . $row['tid'] . "\">del</a></td></tr>";
*/
//echo "<br/>get crdd : " .  $_GET['crdd'];
//echo "<br/>post crdd : " .  $_POST['crdd'];
//echo "<br/>crdd : " .  $row['crdd'];
if (empty($crdd)){$crdd=0;}
//echo "<br/>crdd : " . $crdd;
//echo "<br/>brnd from row : " . $row['brnd'];
	?>
		<form action="post.php" method="post">
		<table>
			<tr>
...
<!--			<td>tran creditor</td><td><input name="crn" value="<?php echo $row['crn']; ?>"></input></td></tr>	-->
			...
			<?php
				...
				//echo "<br/>crdd again" . $crdd;
				//echo fslct($conn,"select id crdd,nm crd from transdet join crd on crdd=id"," where 1=1"," group by crdd,crd order by nm",$crdd,'crdd','crd','func.php');
...
//echo "<br/>crdd: " . $crdd;
//echo "<br/>brnwhr: " . $brnwhr;
//				echo fslct($conn,"select c1.id brnd,c1.nm brn from transdet join crd c1 on brnd=c1.id join crd c2 on c2.id=crdd"," where crdd= " . $crdd," group by brnd,brn order by brn",$brnd,'brnd','brn','func.php');
/////////////////if ($_POST['crdd']==null) {
//echo "<br/> crdd again again : " . $crdd;
//
if ($crdd==null) {
//  $brnwhr = " where creditor_id=0";
  $brnwhr = " where c2.id=0";
...
//
//THIS BUGGERS IT UP	$crdd = $_POST['crdd'];
if (empty($_POST['brnd'])) { $brnd = $row['brnd']; } else { $brnd = $_POST['brnd']; }
//echo "<br/> dollar brnd:" . $brnd;
//				if (empty($crdd)) {
//					$brnwhr = " where branch_id in (select branch_id from transdet where cred_id = 0) ";
//				} else {
//					$brnwhr = " where branch_id in (select branch_id from transdet where cred_id = " . $crdd . ") ";
