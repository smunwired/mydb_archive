...
if ($grp=='all'){
	  ...
	  foreach($conn->query($sql) as $row) {
	    if ($show!=$row['ttyd']) {
			...
			and " . $cls . "=" . $show .
			" and date_format(coalesce(tran_date,statement_date),'%M %Y')='" . date("F Y", strtotime("+" . $_GET['chng'] . " months")) .
			"' order by coalesce(tran_date,statement_date)";


			#echo "<tr><td>" . $sql2 . "</td></tr>";
		echo "<tr><th width=75px>tran date
				<th width=225px>crd<th>desc";
